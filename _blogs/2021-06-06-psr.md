...
Bài toán đặt ra trong trường hợp này là làm như thế nào để tính được đạo hàm trên mạch lượng tử? Giả sử ta có chuỗi các cổng chứa tham số $U(\theta)$ phụ thuộc vào $\theta$ và quan sát $\hat{B}$, phụ thuộc vào quan sát có nghĩa là, chẳng hạn, nếu $\hat{B}$ là ma trận Pauli 

$$
Z=\begin{bmatrix}1 & 0\\0 & -1\end{bmatrix}.$$ 

Thì $\langle 0|\hat{B}|0 \rangle=1$ và $\langle 1|\hat{B}|1 \rangle=-1$  tương ứng là trị riêng của pauli $Z$. $|0 \rangle$ hay $| 1 \rangle$ trong ngữ cảnh này được gọi là trạng thái khởi tạo, trong bài viết này mình sẽ kí hiệu chúng tổng quát là $|\psi\rangle$.

Hàm $f$ cần tìm cực tiểu ở đây có dạng:

$f(\theta):R^m \rightarrow R^n:=\langle \psi | U^\dagger(\theta)\hat{B}U(\theta)| \psi\rangle\;(2.1)$

Về mặt vật lý, chúng ta phải chạy mạch này nhiều lần để lấy trung bình $k$ lần đo là xấp xỉ $f(0)$.
