---
layout: post
title: "Bàn về parameter shift rule"
categories: quantum
---
...
Chiến thuật tìm các điểm cực tiểu được mô tả sau. Chúng ta có hàm $f$ phụ thuộc biến $(x\in X)$ với $x^{\ast}$ là cực tiểu toàn cục và $X=\{x^{(0)},x^{(1)},\ldots,x^{(n)}\}$ là tập các thử nghiệm trong $n$ vòng lặp, $x^{(0)}$ là giá trị khởi tạo, $x^{\ast}$ là giá trị cần tìm, $x$ có thể  là vô hướng, vector, ma trận hoặc đại lượng nhiều chiều hơn. Ở đây ta cần đưa $x\approx x^{\ast}$ càng tốt và $x=x^{\ast}$ là tốt nhất.
...
với $\alpha\in\mathbb{R}$ là hệ số dùng để điều chỉnh tốc độ cập nhật (learning rate) và $\nabla f(x^{(t)})=[\frac{\partial f(x^{(t)})}{\partial x_1} \;\frac{\partial f(x^{(t)})}{\partial x_2}\;\ldots\;\frac{\partial f(x^{(t)})}{\partial x_m}]$ là gradient của $f$ tại $x^{(t)}$.
...
Để khắc phục nhược điểm này, chúng ta có một số phương pháp tính gián tiếp giá trị đạo hàm tại một điểm $f'(x)$ bất kỳ như sai phân hữu hạn, phương pháp này được chứng minh như sau:
...
$\frac{f(x+\epsilon)-f(x-\epsilon)}{2\epsilon}\approx f'(x)+\frac{f^{(3)}(x)}{6}\epsilon^2+\ldots=f'(x)+O(\epsilon^2)\;(1.3)$

</div>
Nếu $\epsilon\approx 0$, ta sẽ tính được gần đúng giá trị $f'(x)$ thông qua hai lần gọi hàm với độ lỗi $O(\epsilon^2)$. Đây còn có thể gọi là phương pháp bậc 0 vì không cần thông qua đạo hàm bậc cao hơn. Đối với mình thì $O(\epsilon^2)$ vẫn quá lớn để chấp nhận, ngoài ra định nghĩa đưa ra chỉ tính được xấp xỉ, có thể tiêu tốn nhiều thời gian tính toán trong trường hợp hàm xấu. Tạm bỏ qua độ lỗi, ta có thể viết lại công thức $(1.3)$ như sau:
...
Bài toán đặt ra trong trường hợp này là làm như thế nào để tính được đạo hàm trên mạch lượng tử? Giả sử ta có chuỗi các cổng chứa tham số $U(\theta)$ phụ thuộc vào $\theta$ và quan sát $\hat{B}$, phụ thuộc vào quan sát có nghĩa là, chẳng hạn, nếu $\hat{B}$ là ma trận Pauli $Z=\begin{bmatrix}1 & 0\\0 & -1\end{bmatrix}$. Thì $\langle 0|\hat{B}|0 \rangle=1$ và $\langle 1|\hat{B}|1 \rangle=-1$  tương ứng là trị riêng của pauli $Z$, $|0\rangle$ hay $| 1 \rangle$ trong ngữ cảnh này được gọi là trạng thái khởi tạo, trong nội dung tiếp theo mình sẽ kí hiệu chúng tổng quát là $|\psi\rangle$.

Hàm $f$ cần tìm cực tiểu ở đây có dạng:

$f(\theta):R^m \rightarrow R^n:=\langle 0 | U^\dagger(\theta)\hat{B}U(\theta)| 0\rangle\;(2.1)$

(với $|\psi\rangle$ trong trường hợp này đơn giản là $| 0 \rangle$). Về mặt vật lý, chúng ta phải chạy mạch này nhiều lần để lấy trung bình $k$ lần đo là xấp xỉ $f(0)$.

Để tiện cho việc đọc hiểu, chúng ta cần biết trước về một số chứng minh.

<div style="border: 2px solid #222; padding: 15px; border-radius: 5px;">

**Chứng minh (2.1)**: Với $B,C$ là hai toán tử bất kỳ, ta xét đại lượng $\langle\psi| B^\dagger\hat{Q}C |\psi\rangle+h.c$
...
$f(\mu)=\langle 0 | U^\dagger\hat{B}U| 0\rangle$

$
=\langle 0 | (V\mathcal{G}(\mu)W)^\dagger\hat{B}(V\mathcal{G}(\mu)W)| 0\rangle
$

$
=\langle 0 | W^\dagger\mathcal{G}^\dagger(\mu)(V^\dagger\hat{B}V)\mathcal{G}(\mu)W| 0\rangle\;(3.1)
$

Hấp thụ $V$ vào đại lượng quan sát $\hat{B}$ và $W$ vào trạng thái $|0\rangle$, ta thế $V^\dagger\hat{B}V=\hat{Q}$, $W| 0\rangle=|\psi\rangle$ và $\langle 0 | W=\langle\psi|$ vào phương trình (3.1):

$f=\langle\psi|\mathcal{G}^{\dagger}\hat{Q}\mathcal{G}|\psi\rangle$

Khi lấy đạo hàm riêng của $f$ đối với $\mu$:

$\partial_{\mu}f=\partial_{\mu}\langle\psi|\mathcal{G}^{\dagger}\hat{Q}\mathcal{G}|\psi\rangle$

$
=\langle\psi|\mathcal{G}^{\dagger}\hat{Q}(\partial_{\mu}\mathcal{G})|\psi\rangle+\langle\psi|(\partial_{\mu}\mathcal{G})^{\dagger}\hat{Q}\mathcal{G}|\psi\rangle\;(3.2)
$
...
$
\partial_{\mu}f=\langle\psi|\mathcal{G}^{\dagger}\hat{Q}(-iG\mathcal{G})|\psi\rangle+\langle\psi|(-iG\mathcal{G})^{\dagger}\hat{Q}\mathcal{G}|\psi\rangle
$

với $\langle\psi|\mathcal{G}^\dagger=\langle\psi^{'}|$ và $\mathcal{G}|\psi\rangle=\;|\psi^{'}\rangle$, $r\in\mathbb{R}$:

$
\partial_{\mu}f=\langle\psi^{'}|\hat{Q}(-iG)|\psi^{'}\rangle+\langle\psi^{'}|(-iG)^{\dagger}\hat{Q}|\psi^{'}\rangle \;
$

$
=r(\langle\psi^{'}|\mathbb{I}^\dagger\hat{Q}(-\frac{i}{r}G)|\psi^{'}\rangle+\langle\psi^{'}|(-\frac{i}{r}G)^{\dagger}\hat{Q}\mathbb{I}|\psi^{'}\rangle) \;(3.4)
$
...
$
\partial_{\mu}f=\frac{r}{2}(\langle\psi^{'}|(\mathbb{I}-\frac{i}{r}G)^\dagger\hat{Q}(\mathbb{I}-\frac{i}{r}G)|\psi^{'}\rangle+\langle\psi^{'}|(\mathbb{I}+\frac{i}{r}G)^\dagger\hat{Q}(\mathbb{I}+\frac{i}{r}G)|\psi^{'}\rangle \;(3.5)
$
...
$
\partial_{\mu}f=\frac{r}{2}(\langle\psi^{'}|\sqrt{2}\mathcal{G}^\dagger(\frac{\pi}{4r})\hat{Q}\sqrt{2}\mathcal{G}(\frac{\pi}{4r})|\psi^{'}\rangle+\langle\psi^{'}|\sqrt{2}\mathcal{G}^\dagger(-\frac{\pi}{4r})\hat{Q}\sqrt{2}\mathcal{G}(-\frac{\pi}{4r})|\psi^{'}\rangle)
$

với $\langle\psi^{'}|=\langle\psi|\mathcal{G}^\dagger$ và $|\psi^{'}\rangle=\mathcal{G}|\psi\rangle$:

$
\partial_{\mu}f=r(\langle\psi|\mathcal{G}^\dagger\mathcal{G}^\dagger(\frac{\pi}{4r})\hat{Q}\mathcal{G}(\frac{\pi}{4r})\mathcal{G}|\psi\rangle+\langle\psi|\mathcal{G}^\dagger\mathcal{G}^\dagger(-\frac{\pi}{4r})\hat{Q}\mathcal{G}(-\frac{\pi}{4r})\mathcal{G}|\psi\rangle)
$

với $\mathcal{G}(\mu+s)=e^{-i(\mu+s)G}=e^{-i\mu G}e^{-isG}=\mathcal{G}(\mu)\mathcal{G}(s)$:

$
\partial_{\mu}f=r(\langle\psi|\mathcal{G}^\dagger(\mu+\frac{\pi}{4r})\hat{Q}\mathcal{G}(\mu+\frac{\pi}{4r})|\psi\rangle+\langle\psi|(\mathcal{G}^\dagger(\mu-\frac{\pi}{4r})\hat{Q}\mathcal{G}(\mu-\frac{\pi}{4r})|\psi\rangle)
$
...
Xét $\partial_\mu f=\langle\psi|(\partial_\mu\mathcal{G}^\dagger)\hat{Q}\mathcal{G}|\psi\rangle+\langle\psi|\mathcal{G}^\dagger\hat{Q}(\partial_\mu\mathcal{G})|\psi\rangle$

$
=\langle\psi|(\frac{i}{2}\sigma_i\mathcal{G}^\dagger)\hat{Q}\mathcal{G}|\psi\rangle+\langle\psi|\mathcal{G}^\dagger\hat{Q}(-\frac{i}{2}\sigma_i\mathcal{G})|\psi\rangle
$

$
=\frac{i}{2}\langle\psi|\mathcal{G}^\dagger(\sigma_i\hat{Q}-\hat{Q}\sigma_i)\mathcal{G}|\psi\rangle
$

$
=\frac{i}{2}\langle\psi|\mathcal{G}^\dagger[\sigma_i,\hat{Q}]\mathcal{G}|\psi\rangle\;(4.2)
$
...
$
=\frac{1}{2}(\langle\psi|\mathcal{G}^\dagger(\mathcal{G}^\dagger(\frac{\pi}{2})\hat{Q}\mathcal{G}(\frac{\pi}{2})-\mathcal{G}^\dagger(-\frac{\pi}{2})\hat{Q}\mathcal{G}(-\frac{\pi}{2}))\mathcal{G}|\psi\rangle)
$

$
=\frac{1}{2}(\langle\psi|\mathcal{G}^\dagger(\mathcal{G}^\dagger(\frac{\pi}{2})\hat{Q}\mathcal{G}(\frac{\pi}{2})\mathcal{G}|\psi\rangle-\langle\psi|\mathcal{G}^\dagger(\mathcal{G}^\dagger(-\frac{\pi}{2})\hat{Q}\mathcal{G}(-\frac{\pi}{2})\mathcal{G}|\psi\rangle)
$

$
=\frac{1}{2}(\langle\psi|\mathcal{G}^\dagger(\mu+\frac{\pi}{2})\hat{Q}\mathcal{G}(\mu+\frac{\pi}{2})|\psi\rangle-\langle\psi|\mathcal{G}^\dagger(\mu-\frac{\pi}{2})\hat{Q}\mathcal{G}(\mu-\frac{\pi}{2})|\psi\rangle)
$
...
Trường hợp 3: một số cổng cơ bản của Xmon qubit trong Google Cirq như $ExpW(\mu,\delta)=e^{-i\mu(\sigma_X\cos{\delta}+\sigma_Y\sin{\delta})}$, $ExpZ(\mu)=e^{-i\mu\sigma_Z}$, $Exp11(\mu)=e^{-i\mu|\ 11 \rangle\langle 11 |}$, ...
